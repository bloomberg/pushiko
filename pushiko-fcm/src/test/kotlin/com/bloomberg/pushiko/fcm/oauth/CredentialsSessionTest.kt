...
internal class CredentialsSessionTest {
    ...
    private val session = CredentialsSession.create(credentials, dispatcher, clock)
    ...
    fun freshTokenDoesNotSwitchDispatcher() = runTest {
        assertEquals(
            "Bearer xyz",
            CredentialsSession.create(credentials, failOnDispatch, clock).currentAuthorization()
        )
    }
    ...
    fun currentAuthorizationUsesLatestToken() = runTest(dispatcher) {
        ...
        val session = CredentialsSession.create(credentials, dispatcher, clock)
    ...
    fun rejectsExpiredOrImminentlyExpiringToken() = runTest(dispatcher) {
        listOf(-1L, 0L, 180L).forEach { expiresInSeconds ->
            ...
            assertFailsWith<IllegalStateException> {
                CredentialsSession.create(credentials, dispatcher, clock).currentAuthorization()
            }
        }
    }
    ...
    @Test
    fun imminentlyExpiringTokenSwitchesToCredentialDispatcher() = runTest {
        var runningOnCredentialDispatcher = false
        var dispatchCount = 0
        val credentialDispatcher = object : CoroutineDispatcher() {
            override fun dispatch(context: CoroutineContext, block: Runnable) {
                dispatchCount++
                runningOnCredentialDispatcher = true
                try {
                    block.run()
                } finally {
                    runningOnCredentialDispatcher = false
                }
            }
        }
        val credentials = mock<ServiceAccountCredentials>().apply {
            ...
            whenever(refreshIfExpired()).thenAnswer {
                assertTrue(runningOnCredentialDispatcher)
            }
        }

        assertEquals(
            "Bearer fresh",
            CredentialsSession.create(credentials, credentialDispatcher, clock).currentAuthorization()
        )
    ...
    fun constructorDoesNotRefreshCredentials() {
        ...
        CredentialsSession.create(credentials, dispatcher)
        verify(credentials, never()).refreshIfExpired()
